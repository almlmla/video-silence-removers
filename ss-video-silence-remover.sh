...
perform_cut() {
    ...
    if (( $(echo "$end > $start" | bc -l) )); then
        ...
        ffmpeg -hide_banner -loglevel error -y -ss "$start" -to "$end" -i "$file" -c copy -avoid_negative_ts make_zero "${cut_file}"
        cut_files+=("$cut_file")
    fi
}
...
for file in *.mp4; do
    if [ -f "$file" ]; then
        ...
        # Perform initial cut for the first second
        perform_cut "0" "1" "$file" "$num_cuts"
        num_cuts=$((num_cuts+1))
        ...
        outfile="${full_filename}_trimmed.${file##*.}"
